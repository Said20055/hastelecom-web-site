...
def auth(client, email, password):
    r = client.post('/crm/api/auth/login', {'email': email, 'password': password}, content_type='application/json')
    return r.json()['access']


def test_login_refresh_flow(client, operator):
    res = client.post('/crm/api/auth/login', {'email': operator.email, 'password': 'Operator123!'}, content_type='application/json')
    assert res.status_code == 200
    assert 'access' in res.json()
    refresh = res.cookies['refresh_token'].value
    res2 = client.post('/crm/api/auth/refresh', HTTP_COOKIE=f'refresh_token={refresh}')
    assert res2.status_code == 200


def test_create_ticket(client, user):
    token = auth(client, user.email, 'User123!')
    res = client.post('/crm/api/tickets', {
        'type': 'support', 'address': 'Addr', 'contactName': 'A', 'phone': '+1', 'description': 'desc'
    }, content_type='application/json', HTTP_AUTHORIZATION=f'Bearer {token}')
    assert res.status_code == 201


def test_rbac_ticket_visibility(client, operator, tech, user):
    t = Ticket.objects.create(type='support', address='A', contactName='C', phone='1', description='d', createdBy=user, assignedTo=tech)
    token = auth(client, tech.email, 'Tech123!')
    res = client.get('/crm/api/tickets', HTTP_AUTHORIZATION=f'Bearer {token}')
    assert res.status_code == 200
    assert len(res.json()) == 1 and res.json()[0]['id'] == t.id


def test_public_ticket_endpoint(client, db):
    res = client.post('/crm/api/public/tickets', {
...
def test_assign_tech_and_status_change(client, operator, tech, user):
    ...
    res = client.patch(f'/crm/api/tickets/{ticket.id}', {'assignedTo': tech.id, 'status': 'in_progress'}, content_type='application/json', HTTP_AUTHORIZATION=f'Bearer {token}')
    assert res.status_code == 200
    assert res.json()['assignedTo'] == tech.id


def test_public_lists(client, db):
    Tariff.objects.create(title='T1', speedMbps=100, price=500, type='home', features=['x'])
    Announcement.objects.create(title='A1', body='b', level='info', startsAt=timezone.now()-timedelta(hours=1), endsAt=timezone.now()+timedelta(hours=1), placement='homepage')
